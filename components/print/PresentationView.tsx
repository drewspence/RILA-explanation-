...
export function PresentationView({
  ...
  const points = payoffData.filter((p) => p.market % 0.05 === 0);
  ...
  return (
    <section className="space-y-4 print:space-y-2">
      <div className="no-print rounded-xl border border-slate-200 bg-white p-4">
        ...
        <input value={clientName} onChange={(e) => setClientName(e.target.value)} className="mt-2 w-full rounded-xl border border-slate-300 p-2 text-sm" />
      </div>

      <article className="rounded-[24px] border border-slate-200 bg-white p-7 shadow-lg print:rounded-none print:border-none print:shadow-none">
        <header className="border-b border-slate-200 pb-4">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Advisor / Client Illustration</p>
          <h2 className="mt-2 text-3xl font-semibold text-slate-900">{strategy}</h2>
          <p className="mt-1 text-sm text-slate-600">Prepared for: <strong>{clientName || "Client Name"}</strong></p>
        </header>
        ...
        <section className="mt-5 grid gap-4 lg:grid-cols-[1.3fr_0.7fr]">
          <div className="rounded-2xl border border-slate-300 bg-slate-950 p-4">
            <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-400">Simplified payoff visual</p>
            <svg viewBox="0 0 420 190" className="h-48 w-full">
              <rect x={0} y={0} width={420} height={190} fill="#020617" rx={8} />
              <line x1={30} y1={95} x2={390} y2={95} stroke="#334155" strokeDasharray="5 5" />
              <line x1={210} y1={20} x2={210} y2={170} stroke="#334155" strokeDasharray="5 5" />
              <path
                d={toPath(points)}
                stroke="#60a5fa"
                strokeWidth={4}
                fill="none"
              />
              <text x={36} y={28} fill="#94a3b8" fontSize="10">{strategyId} payoff profile</text>
            </svg>
          </div>
        ...
        <section className="mt-5 rounded-xl border border-slate-200 bg-white p-4">
          ...
          <p className="mt-2 text-sm text-slate-700">This strategy links returns to index behavior with defined downside rules and defined upside tradeoffs. In this scenario, a market move of {pct(marketReturn)} leads to a credited return of {pct(result.creditedReturn)}, producing an estimated ending value of {currency(result.endingValue)} before fees and riders.</p>
        </section>
...
function toPath(points: Array<{ market: number; credited: number }>) {
  const x = (value: number) => 30 + ((value + 0.5) / 1.0) * 360;
  const y = (value: number) => 170 - ((value + 0.5) / 1.0) * 150;
  return points.map((p, i) => `${i === 0 ? "M" : "L"}${x(p.market)},${y(p.credited)}`).join(" ");
}
